...
namespace RPGMaker.Codebase.Editor.Hierarchy.Region.AssetManage
{
    ...
    public class AssetManageHierarchy : AbstractHierarchy
    {
        ...
        public void CreateAssetManageDataModel(AssetCategoryEnum assetType) {
            ...
            /*
             【不具合（調査／修正）】フィールドキャラクタをすべて削除すると追加できなくなる
              ...
              上記不具合の修正で派生で発覚した不具合
              新規の表示名の番号のみ合わせる…_orderData でずれる場合があるため…
              ずれる原因は、_orderDataにデータがあって、JSON/Assets/配下にデータがない場合がなぜか発生するに表示上の番号がずれる為           
            */
            int Count = GetAssetManageDataModeCount(assetType);
            var newModel = AssetManageDataModel.CreateDefault((int) assetType, Count);
            newModel.name = "#" + string.Format("{0:D4}", (Count + 1)) + "　" + EditorLocalize.LocalizeText("WORD_1518");
        ...
        /// <param name="assetType"></param>
        /// <returns></returns>
        public int GetAssetManageDataModeCount(AssetCategoryEnum assetType) 
        {
            int Count = 0;

            switch (assetType)
            {
                case AssetCategoryEnum.MOVE_CHARACTER:
                    Count = _walkingCharacterAssets.Count;
                    break;
                case AssetCategoryEnum.OBJECT:
                    Count = _objectAssets.Count;
                    break;
                case AssetCategoryEnum.SV_BATTLE_CHARACTER:
                    Count = _actorAssets.Count;
                    break;
                case AssetCategoryEnum.POPUP:
                    Count = _popupIconAssets.Count;
                    break;
                case AssetCategoryEnum.SV_WEAPON:
                    Count = _weaponAssets.Count;
                    break;
                case AssetCategoryEnum.SUPERPOSITION:
                    Count = _stateOverlapAssets.Count;
                    break;
                case AssetCategoryEnum.BATTLE_EFFECT:
                    Count = _battleEffectAssets.Count;
                    break;
            }
            return Count;
        }
