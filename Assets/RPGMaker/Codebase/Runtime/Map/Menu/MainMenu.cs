...
#else
        public async Task Init(WindowBase manager) {
#endif
...
#else
                            await characterItem.Init(saveData.runtimeActorDataModels[j]);
#endif
...
#else
        private async Task SortOpen() {
            await _menuBase.SortOpen();
        }
#endif
...
#else
        private async Task OptionOpen() {
#endif
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
            _menuBase.OptionOpen();
#else
            await _menuBase.OptionOpen();
#endif
        }
...
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
            _ = CharacterEventAsync(obj);
        }
        public async Task CharacterEventAsync(GameObject obj) {
#endif
            var component = obj.GetComponent<CharacterItem>();
            if(component == null)
            {
                return;
            }
            var actorId = component.ActorId();
            switch (_display)
            {
...
#else
                    await _menuBase.SkillOpen(actorId);
#endif
                    break;
...
#else
                    await _menuBase.EquipmentOpen(actorId);
#endif
                    break;
...
#else
                    await _menuBase.StatusOpen(actorId);
#endif
                    break;
            }
