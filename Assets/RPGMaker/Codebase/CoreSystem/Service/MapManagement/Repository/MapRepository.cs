...
namespace RPGMaker.Codebase.CoreSystem.Service.MapManagement.Repository
{
    public class MapRepository
    {
...
#else
        public async Task<List<MapDataModel>> LoadMapDataModels() {
        ...
        public List<MapDataModel> LoadMapDataModelsSync() {
            if (_mapDataModels != null)
            {
                // キャッシュがあればそれを返す
                return _mapDataModels;
            ...
            _mapDataModels = GetJsonsSync(JsonFile).Select(ConvertJsonToEntity).ToList();
...
#else
        public async Task<List<MapBaseDataModel>> LoadMapBaseDataModels() {
#endif
#if UNITY_EDITOR && !UNITE_WEBGL_TEST
            // 再生中でなければ
...
#else
            _mapBaseDataModels = (await ScriptableObjectOperator.GetClass<MapBaseDataModel>(JsonBaseFile)) as List<MapBaseDataModel>;
#endif
#endif
...
#else
        public async Task<MapDataModel> LoadMapDataModel(string id, bool isSampleMap) {
#endif
#if UNITY_EDITOR && !UNITE_WEBGL_TEST
            if (!isSampleMap)
            {
...
#else
            return (await ScriptableObjectOperator.GetClass<MapDataModel>(JsonFile + id + ".json", true)) as MapDataModel;
#endif
#endif
        }
...
#endif
#if UNITY_EDITOR && UNITY_WEBGL
        private List<MapJson> GetJsonsSync(string path) {
            var mapJson = new List<MapJson>();
            var files = Directory.GetFiles(path, "*.json", SearchOption.TopDirectoryOnly);

            foreach (var file in files)
                mapJson.Add(
                    JsonHelper.FromJson<MapJson>(UnityEditorWrapper.AssetDatabaseWrapper.LoadJsonStringSync(file)));
...
#else
            _mapSampleDataModels = (await ScriptableObjectOperator.GetClass<MapDataModel>(JsonFile)) as List<MapDataModel>;
#endif
#endif
