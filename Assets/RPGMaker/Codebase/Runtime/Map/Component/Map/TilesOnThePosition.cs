...
#else
        public async Task InitForRuntime(MapDataModel mapDataModel, Vector2 position) {
#endif
...
#else
            _tileA = (await mapPrefabManager.GetLayerByType(MapDataModel.Layer.LayerType.A)).GetTileDataModelByPosition(position);
            _tileB = (await mapPrefabManager.GetLayerByType(MapDataModel.Layer.LayerType.B)).GetTileDataModelByPosition(position);
            _tileC = (await mapPrefabManager.GetLayerByType(MapDataModel.Layer.LayerType.C)).GetTileDataModelByPosition(position);
            _tileD = (await mapPrefabManager.GetLayerByType(MapDataModel.Layer.LayerType.D)).GetTileDataModelByPosition(position);
            _tileCollider = (await mapPrefabManager.layers())[(int) MapDataModel.Layer.LayerType.BackgroundCollision].GetTileDataModelByPosition(position);
            _tileRegion = (await mapPrefabManager.layers())[(int) MapDataModel.Layer.LayerType.Region].GetTileDataModelByPosition(position);
#endif
        }
