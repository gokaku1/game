...
#else
        public async Task<List<Layer>> LayersForRuntime() {
            await UniteTask.Delay(0);
            return await MapPrefabManagerForRuntime.layers();
        }
#endif
...
#else
        public static async Task<GameObject> InstantiateMapPrefab(MapDataModel mapDataModel, GameObject mapPrefab)
        {
            Debug.Log($"InstantiateMapPrefab: {mapDataModel.name}, {mapDataModel.id}");
            return await new MapPrefabManager(mapDataModel, Object.Instantiate(mapPrefab)).CorrectionMapPrefab();
        }
#endif
...
#else
        public async Task<Layer> GetLayerByTypeForRuntime(Layer.LayerType layerType)
#endif
        {
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
            return MapPrefabManagerForRuntime.GetLayerByType(layerType);
#else
            return await MapPrefabManagerForRuntime.GetLayerByType(layerType);
#endif
        }
...
#else
        public async Task<Transform> GetLayerTransformForRuntime(Layer.LayerType layerType)
#endif
        {
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
            return MapPrefabManagerForRuntime.GetLayerTransform(layerType);
#else
            return await MapPrefabManagerForRuntime.GetLayerTransform(layerType);
#endif
        }
...
#else
            public async Task<List<TileDataModel>> tiles() { return await ToTileData(); }
#endif
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
            private List<TileDataModel> ToTileData() {
#else
            private async Task<List<TileDataModel>> ToTileData() {
#endif
...
#else
                    tile.Add(await tilesOnPalette[i].TileDataModel());
#endif
                }
                return tile;
            }
...
#else
            public async Task<List<Layer>> layers() {
...
#else
            public async Task<Transform> GetLayerTransform(Layer.LayerType layerType)
#endif
            {
...
#else
                return (await LoadPrefab()).transform.GetChild(childIndex);
#endif
            }
...
#else
            public async Task<GameObject> LoadPrefab(bool isSampleMap = false)
#endif
            {
                if (_mapPrefab == null)
                {
...
#else
                        await UnityEditorWrapper.PrefabUtilityWrapper.LoadPrefabContents(
                            mapDataModel.GetPrefabPath(isSampleMap), true);
#endif
...
#else
            public async Task<List<Layer>> LoadLayers()
#endif
            {
                if (_mapLayers != null)
                {
...
#else
                var mapPrefab = await LoadPrefab();
#endif
#if UNITY_EDITOR
...
#if UNITY_EDITOR && !UNITE_WEBGL_TEST    // Runtime時はパレット情報不要
...
#else
            public async Task<GameObject> CorrectionMapPrefab()
#endif
            {
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                foreach (Layer layer in layers)
#else
                foreach (Layer layer in await layers())
#endif
                {
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                    var layerTransform = GetLayerTransform(layer.type);
#else
                    var layerTransform = await GetLayerTransform(layer.type);
#endif
...
#else
            public async Task<Layer> GetLayerByType(Layer.LayerType type)
#endif
            {
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                var layers = LoadLayers();
#else
                var layers = await LoadLayers();
#endif
