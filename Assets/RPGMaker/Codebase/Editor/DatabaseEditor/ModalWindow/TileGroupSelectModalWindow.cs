...
namespace RPGMaker.Codebase.Editor.DatabaseEditor.ModalWindow
{
    public class TileGroupSelectModalWindow : BaseModalWindow
    {
        ...
        public override void Init() {
            ...
            buttonOk.clicked += () =>
            {
                if (_tileGroupEntities != null && _tileGroupEntities.Count != 0)
                {
                    if (EditorUtility.DisplayDialog(
                          ...
                          EditorLocalize.LocalizeText(_isSave ? "WORD_0788" : "WORD_0789"),
                          EditorLocalize.LocalizeText("WORD_1652"),
                          EditorLocalize.LocalizeText("WORD_2901"),
                          EditorLocalize.LocalizeText("WORD_5020")) == true)
                    {
                        // 確定時に引数のタイルグループを保存
                        if (_isSave)
                        {
            ...
            buttonOk.text = EditorLocalize.LocalizeText(_isSave ? "WORD_0788" : "WORD_0789");
            buttonCancel.text = EditorLocalize.LocalizeText("WORD_5020");
        }
        ...
        private Button CreateAddButton(VisualElement visualElement) {
            ...
            button.text = EditorLocalize.LocalizeText("WORD_1613");
            button.clicked += () =>
            {
                // データを作成して追加、セーブする
                _mapManagementService.SaveTileGroup(new TileGroupDataModel(
                    Guid.NewGuid().ToString(), EditorLocalize.LocalizeText("WORD_1596"), new List<TileDataModelInfo>(), false));
                _tileGroupEntities = _mapManagementService.LoadTileGroups();
                visualElement.Clear();
