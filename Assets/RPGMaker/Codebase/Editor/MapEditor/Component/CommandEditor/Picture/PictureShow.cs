...
namespace RPGMaker.Codebase.Editor.MapEditor.Component.CommandEditor.Picture
{
    ...
    public class PictureShow : AbstractCommandEditor
    {
        ...
        public override void Invoke() {
            ...
            // ピクチャクリック時に発火するコモンイベント一覧
            VisualElement executeCommonEvent = RootElement.Query<VisualElement>("executeCommonEvent");
            var eventCommonDataModels = EventManagementService.LoadEventCommon();
            var selectIndex = 0;
            if (_targetCommand.parameters.Count >= 11)
            {
                // コモンイベントが設定されていたら値をセット
                for (var i = 0; i < eventCommonDataModels.Count; i++)
                {
                    if (eventCommonDataModels[i].eventId == _targetCommand.parameters[10])
                    {
                        selectIndex = i + 1;
                        break;
                    }
            ...
            {
                // コモンイベントが設定されていなければ0をセット
                _targetCommand.parameters.Add("0");
            }

            var choices = new List<string>() { EditorLocalize.LocalizeText("WORD_0113")};
            eventCommonDataModels.ForEach(eventCommonDataModel => choices.Add(eventCommonDataModel.name));
            var commonEventPopupField = new PopupFieldBase<string>(choices, selectIndex);
            commonEventPopupField.RegisterValueChangedCallback(evt =>
            {
                _targetCommand.parameters[10] = commonEventPopupField.index == 0 ? "0" : eventCommonDataModels[commonEventPopupField.index - 1].eventId;
                Save(EventDataModels[EventIndex]);
            });
            executeCommonEvent.Add(commonEventPopupField);

            // ピクチャクリック時に透過を許容するかどうか
            Toggle executeInvisibleClickFlg = RootElement.Query<Toggle>("executeInvisibleClickFlg");
            if (_targetCommand.parameters.Count <= 12)
            {
                _targetCommand.parameters.Add("0");
            }
            executeInvisibleClickFlg.value = _targetCommand.parameters[11] == "1";
            executeInvisibleClickFlg.RegisterValueChangedCallback(evt =>
            {
                _targetCommand.parameters[11] = executeInvisibleClickFlg.value ? "1" : "0";
                Save(EventDataModels[EventIndex]);
            });
            ...
            {
                var selectModalWindow = ImagePicker.Instantiate(PathManager.IMAGE_ADV, true);
