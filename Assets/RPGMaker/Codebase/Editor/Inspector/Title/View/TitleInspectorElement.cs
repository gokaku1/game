...
namespace RPGMaker.Codebase.Editor.Inspector.Title.View
{
    ...
    public class TitleInspectorElement : AbstractInspectorElement
    {
        ...
        override protected void InitializeContents() {
            ...
            {
                var backgroundSelectModalWindow = ImagePicker.Instantiate(PathManager.UI_TITLE_BG);
                backgroundSelectModalWindow.ShowWindow(EditorLocalize.LocalizeWindowTitle("WORD_0101"), data =>
                {
                    ...
                    _UpdateSceneView();
                }, _runtimeTitleDataModel.titleBackgroundImage, true);
            };
            // 背景画像インポート
            ...
            {
                var frontSelectModalWindow = ImagePicker.Instantiate(PathManager.UI_TITLE_FRAME);
                frontSelectModalWindow.ShowWindow(EditorLocalize.LocalizeWindowTitle("WORD_0102"), data =>
                {
                    ...
                    _UpdateSceneView();
                }, _runtimeTitleDataModel.titleFront.image, true);
            };
            // 装飾画像インポート
            ...
            titleTitlenameTextFont.showAlpha = false;
            //Unity6で問題発生するので下記設定を追加
            titleTitlenameTextFont.style.flexGrow = 1.0f;

            //U313 カラーを即反映する様に修正
            titleTitlenameTextFont.RegisterValueChangedCallback(evt =>
            {
                var r = evt.newValue.r * 255.0f;
                var g = evt.newValue.g * 255.0f;
                var b = evt.newValue.b * 255.0f;
            ...
            {
                var backgroundSelectModalWindow = ImagePicker.Instantiate(PathManager.UI_TITLE_NAME);
            ...
            titleFontsettingColor.showAlpha = false;
            //U356 カラー設定を行なえる様に修正
            titleFontsettingColor.style.flexGrow = 1.0f;
            //U313 カラーを即反映する様に修正
            titleFontsettingColor.RegisterValueChangedCallback(evt =>
            {
                var r = evt.newValue.r * 255.0f;
                var g = evt.newValue.g * 255.0f;
                var b = evt.newValue.b * 255.0f;
