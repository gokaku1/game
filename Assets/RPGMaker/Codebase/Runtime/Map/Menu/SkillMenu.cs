    ...
    public class SkillMenu : WindowBase
    {
        ...
        public async Task Init(MenuBase @base, string actorId) {
...
#else
                    obj.transform.GetComponent<WindowButtonBase>().OnClick.AddListener(() => _ = CreateItem(index));
#endif
                }
            ...
            for (int i = 0; i < partyActors.Count; i++)
                if (partyActors[i].ActorId == _runtimeActorDataModel.actorId)
#endif
                {
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                    _actor = party.Actors[i];
#else
                    _actor = partyActors[i];
#endif
                    break;
                }
...
#else
        public async Task CreateItem(int type) {
#endif
            if (_skillItems.Count > 0)
                foreach (var s in _skillItems)
                    Destroy(s.gameObject);
...
#else
                await skill.Init(this, skillData[i].basic.id, OpenParty, CloseParty);
                obj.SetActive(true);
#endif
                if (disabledSkill)
                {
...
#else
                await skill.Init(this, "", null, CloseParty);
                obj.SetActive(true);
#endif
                _skillItems.Add(skill);
            }
...
#else
                await _skillCommand.CharacterChangeAsync(isNext);
#endif
        }
    }
}