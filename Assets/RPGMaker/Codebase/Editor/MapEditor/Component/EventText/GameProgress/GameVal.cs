        ...
        public override VisualElement Invoke(string indent, EventDataModel.EventCommand eventCommand) {
            ...
            else if (int.Parse(eventCommand.parameters[3]) == 3)
            {
                ...
                else if (path[0] == "4")
                {
                    ...
                    //2文字以上の場合は、対応出来ないので、初期化しておく 0-7 までの配置数しかないはずなので…
                    if (path[1].Length >= 2)
                    {
                        path[1] = "0";
                    }
                    //バルト時のグループ配置から表示名を取得する U117
                    string Name = GameVariable.GetBattleEnemyName(int.Parse(path[1]));
                    ret += EditorLocalize.LocalizeText(enemyStatus[int.Parse(path[2])]) + " of # " + Name;
                }
                else if (path[0] == "5")
                {
                    var name = "";
