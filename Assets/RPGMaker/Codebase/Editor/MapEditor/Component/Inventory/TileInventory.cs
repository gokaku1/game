...
namespace RPGMaker.Codebase.Editor.MapEditor.Component.Inventory
{
    ...
    public class TileInventory : VisualElement
    {
        ...
        public TileInventory(
            ...
            Action<TileDataModel> onSelectTile,
            Action<TileDataModel.Type> onChangeTileType,
            Action<TileDataModel> onDoubleClickTile,
            List<TileDataModel.Type> activeTypes,
            TileDataModel.Type type,
            string tileId
        ) {
            Type = TileInventoryType.TileSelection;
            TileEntities = tileEntities;
            _onSelectTile = onSelectTile;
            _onChangeTileType = onChangeTileType;
            InitUI(activeTypes);
            _tileListView.SetReselectCurrent(true);
            ChangeTileType(type);
            var selectingTile = TileEntities.Find(x => x.id == tileId);
            if (selectingTile != null)
            {
                _tileListView.SelectAndScrollToTile(selectingTile.TileDataModel);
            }
            _tileListView.SetDoubleClickTileCallback(onDoubleClickTile);
        }
         ...
         * UI初期化
         */
        private void InitUI(List<TileDataModel.Type> activeTypes) {
            ...
            foreach (var tabTileType in TabTileTypes)
            {
                ...
                if (activeTypes != null && !activeTypes.Contains(tabTileType))
                {
                    //activeTypesに含まれないタイルタイプボタンは非表示にする。
            ...
            _tileListView = new TileListView(
                GetTilesOfCurrentType(), SelectTile, null, isDoubleCursor: Type == TileInventoryType.TileEdit || Type == TileInventoryType.TileGroupEdit || Type == TileInventoryType.TileSelection);
