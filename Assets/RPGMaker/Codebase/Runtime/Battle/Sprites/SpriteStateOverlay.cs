...
#else
        public async Task<bool> LoadBitmap() {
#endif
            //画像のAssetDataを読込
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
            InitData();
...
#else
            assetData = (await ScriptableObjectOperator.GetClass<AssetManageDataModel>("SO/" + _overlayImageId + ".asset")) as AssetManageDataModel;
#endif
#endif
        }
...
#else
        private async Task MotionAnimation() {
#endif
...
#else
                await UniteTask.Delay(Mathf.FloorToInt(waitTime * (1000f / 60f)));
#endif
