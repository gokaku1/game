...
namespace RPGMaker.Codebase.CoreSystem.Service.MapManagement
{
    public class MapRenderingOrderManager
    {
        ...
        //乗り物レイヤーからソートレイヤーを取得する
        public static int GetVehicleSortingLayerId(VehiclesDataModel.LayerType LayerType)
        {
            UnityUtil.SortingLayerManager.SortingLayerIndex RetLayer = UnityUtil.SortingLayerManager.SortingLayerIndex.Default;
            switch (LayerType)
            {
                case VehiclesDataModel.LayerType.A:
                    // マップレイヤーA。
                    RetLayer = UnityUtil.SortingLayerManager.SortingLayerIndex.MapLayerA;
                    break;
                case VehiclesDataModel.LayerType.B:
                    // マップレイヤーB。
                    RetLayer = UnityUtil.SortingLayerManager.SortingLayerIndex.MapLayerB;
                    break;
                case VehiclesDataModel.LayerType.C:
                    // マップレイヤーC。
                    RetLayer = UnityUtil.SortingLayerManager.SortingLayerIndex.MapLayerC;
                    break;
                case VehiclesDataModel.LayerType.D:
                    // マップレイヤーD。
                    RetLayer = UnityUtil.SortingLayerManager.SortingLayerIndex.MapLayerD;
                    break;
                case VehiclesDataModel.LayerType.CharacterPriorityNormal:
                    // キャラクター プライオリティ 通常。
                    RetLayer = UnityUtil.SortingLayerManager.SortingLayerIndex.Runtime_MapCharacterPriorityNormal;
                    break;
                case VehiclesDataModel.LayerType.CharacterPriorityHigh:
                    // 飛行中のキャラクター プライオリティ 上。
                    RetLayer = UnityUtil.SortingLayerManager.SortingLayerIndex.Runtime_MapCharacterInFlyingPriorityUpper;
                    break;

            }
            return UnityUtil.SortingLayerManager.GetId(RetLayer);
        }
    }
}
