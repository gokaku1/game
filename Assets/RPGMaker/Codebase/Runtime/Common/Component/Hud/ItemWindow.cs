    ...
    public class ItemWindow : WindowBase
    {
        ...
        private async Task SetupBelongingList() {
...
#else
                            await CreateContent(i, contentParent[0], item)
#endif
                        );
                }
            }
            ...
            if (itemList.Count == 0)
            {
                ...
                itemList[0].SetText("","","");
#else
                itemList.Add(await CreateContent(0, contentParent[0], ItemDataModel.CreateDefault("")));
                await itemList[0].SetText("","","");
#endif
            }
...
#else
        public async Task<ItemShopContent> CreateContent<T>(
#endif
            int i,
            Transform parent,
            T itemData
...
#else
            await ItemDataSetting(itemContent, i, itemData);
#endif
            return itemContent;
        }
        ...
        async Task ItemDataSetting<T>(
...
#else
            await itemContent.SetTextColor(itemBasic.name, itemContent.ItemCount.ToString(), itemBasic.iconId,
#endif
            ...
            //エデッタ側とフォントのサイズの計算を合せる…
            //Assets\RPGMaker\Codebase\Editor\DatabaseEditor\View\Preview\TalkWindowPreview.cs 505行目当たり
            //フォントサイズの設定 不具合(Unite_164)
            // FontSize設定
            var iFontSetting = DataManager.Self().GetUiSettingDataModel().talkMenu.itemSelectMenu.menuFontSetting;
            var iUniteFontResize = new FontSize(iFontSetting.size);
            itemContent.SetFontSize(iUniteFontResize.ComponentFontSize);
            //フォントサイズを指定のサイズに設定するため 不具合(Unite_164)
            itemContent.SetFontBestFit(false);
        }
