...
#else
            await _stateIconSprite.Initialize();
#endif
...
#else
                await LoadBitmap(_enemy.Enemy.images.image, hue);
#endif
                InitVisibility();
            }
        }
...
#else
        public async Task LoadBitmap(string name, int hue) {
            bitmap = await ImageManager.LoadEnemy(name, hue);
        }
#endif
