...
namespace RPGMaker.Codebase.Runtime.Addon
{
    ...
    public class AddonInstanceContainer : List<AddonInstance> {
        ...
        public new void Add(AddonInstance addonInstance) {
...
#else
        public static async Task<string> ValidateValue(AddonParamInfo paramInfo, ParamType paramType, string valueStr) {
#endif
...
#else
                        var eventCommonDataModels = await new EventManagementService().LoadEventCommon();
#endif
                        int serialNumber = -1;
...
#else
                        var mapEntities = await new MapManagementService().LoadMaps();
#endif
                        int serialNumber = -1;
...
#else
                        var eventMapDataModels = await new EventManagementService().LoadEventMap();
#endif
...
#else
                        var flags = await new DatabaseManagementService().LoadFlags();
#endif
                        int serialNumber = -1;
                        if (!int.TryParse(valueStr, out serialNumber))
                        {
...
#else
                        var animations = await new DatabaseManagementService().LoadAnimation();
#endif
                        int serialNumber = -1;
                        if (!int.TryParse(valueStr, out serialNumber))
                        {
...
#else
                        var skills = await new DatabaseManagementService().LoadSkillCustom();
#endif
                        int serialNumber = -1;
                        if (!int.TryParse(valueStr, out serialNumber))
                        {
...
#else
                        var items = await new DatabaseManagementService().LoadItem();
#endif
                        int serialNumber = -1;
                        if (!int.TryParse(valueStr, out serialNumber))
                        {
...
#else
                        var enemies = await new DatabaseManagementService().LoadEnemy();
#endif
                        int serialNumber = -1;
                        if (!int.TryParse(valueStr, out serialNumber))
                        {
...
#else
                        var troops = await new DatabaseManagementService().LoadTroop();
#endif
                        int serialNumber = -1;
                        if (!int.TryParse(valueStr, out serialNumber))
                        {
...
#else
                        var states = await new DatabaseManagementService().LoadStateEdit();
#endif
                        int serialNumber = -1;
                        if (!int.TryParse(valueStr, out serialNumber))
                        {
...
#else
                        var tilesets = await new MapManagementService().LoadTileGroups();
#endif
                        int serialNumber = -1;
                        if (!int.TryParse(valueStr, out serialNumber))
                        {
...
#else
        public static async Task<JSONNode> GetJsonValue(AddonInfo addonInfo, AddonParamInfo paramInfo, string typeName, string valueStr) {
#endif
            int arrayDimension = 0;
            string structName = null;
            var paramType = GetParamType(typeName, out arrayDimension, out structName);
            if (arrayDimension == 2)
            {
                ...
                for (int i = 0; i < jsonArray.Count; i++)
                {
                    ...
                    for (int j = 0; j < jsonArray2.Count; j++)
                    {
                        ...
                        jsonArray2[j] = GetJsonValue(addonInfo, paramInfo, baseTypeName, IsStringType(baseTypeName) ? jsonArray2[j].Value : jsonArray2[j].ToString());
#else
                        jsonArray2[j] = await GetJsonValue(addonInfo, paramInfo, baseTypeName, IsStringType(baseTypeName) ? jsonArray2[j].Value : jsonArray2[j].ToString());
#endif
                    }
                    jsonArray[i] = jsonArray2;
                }
            ...
            else if (arrayDimension == 1)
            {
                ...
                for (int i = 0; i < jsonArray.Count; i++)
                {
                    ...
                    jsonArray[i] = GetJsonValue(addonInfo, paramInfo, baseTypeName, IsStringType(baseTypeName) ? jsonArray[i].Value : jsonArray[i].ToString());
#else
                    jsonArray[i] = await GetJsonValue(addonInfo, paramInfo, baseTypeName, IsStringType(baseTypeName) ? jsonArray[i].Value : jsonArray[i].ToString());
#endif
                }
                return jsonArray;
            }
...
#else
                        jsonObject[key] = await GetJsonValue(addonInfo, paramParamInfo, tn, IsStringType(tn) ? jsonObject[key].Value : jsonObject[key].ToString());
#endif
                    }
                }
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
...
#else
                return await GetJsonValue(paramInfo, paramType, valueStr);
#endif
            }
        }
...
#else
        public static async Task<JSONNode> GetJsonValue(AddonParamInfo paramInfo, ParamType paramType, string valueStr) {
#endif
            var minParam = paramInfo.GetInfo("min");
            var maxParam = paramInfo.GetInfo("max");
            int arrayDimension = 0;
                ...
                case ParamType.Actor:
                    {
                        ...
                        var actors = new DatabaseManagementService().LoadCharacterActor();
#else
                        var actors = await new DatabaseManagementService().LoadCharacterActor();
#endif
                        int serialNumber = -1;
                        if (!int.TryParse(valueStr, out serialNumber))
                        {
...
#else
            public async Task<AddonInfoContainer> ScanAndMergeAddons() {
#endif
                var modified = false;
...
#else
                            await AdjustInvalidParameter(addonInfo);
#endif
...
#else
            private async Task AdjustInvalidParameter(AddonInfo addonInfo) {
#endif
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
...
#else
                    await AdjustInvalidParameter(addonInfo, paramInfo);
#endif
                }
                foreach (var commandInfo in addonInfo.commandInfos)
                {
...
#else
                        await AdjustInvalidParameter(addonInfo, arg);
#endif
                    }
                }
...
#else
                        await AdjustInvalidParameter(addonInfo, param);
#endif
                    }
                }
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
...
#else
            private async Task AdjustInvalidParameter(AddonInfo addonInfo, AddonParamInfo paramInfo, AddonParameter typeInfo = null, AddonParameter defaultInfo = null) {
#endif
                if (defaultInfo == null)
                {
                    defaultInfo = paramInfo.GetInfo("default");
...
#else
                                    await AdjustInvalidParameter(addonInfo, paramInfo, newTypeInfo, newDefaultInfo);
#endif
...
#else
                                    newDefaultInfo.value = await ValidateValue(paramInfo, paramType2, "");
#endif
                                }
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                                AdjustInvalidParameter(addonInfo, paramInfo, newTypeInfo, newDefaultInfo);
...
#else
                            defaultInfo.value = await ValidateValue(addonInfo, paramInfo, $"struct<{structName}>", defaultInfo.value);
#endif
                            break;
                        }
