    ...
    public class EventCommandChainLauncher : AbstractEventCommandChainLauncher
    {
...
#else
        protected Func<Task<bool>> _checkValid;
#endif
...
#else
        public EventCommandChainLauncher(Func<Task<bool>> checkValid = null) {
#endif
            _eventManagementService = new EventManagementService();
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
        }
        public async Task InitForConstructor(Func<Task<bool>> checkValid = null) {
#endif
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
        ...
        public override async Task ProcessCommandAsync(string eventId) {
            ...
            if (_isCallback && 
                ...
                eventCodeBef == (int) EventEnum.EVENT_CODE_SCENE_CALL_UNITY_SCENE ||//Unityシーンを呼び出す
                //U376 場所移動時は、１つしか実行できない命令へ
            ...
            if (!_isCallback &&
                ...
                _eventCode == (int) EventEnum.EVENT_CODE_SCENE_CALL_UNITY_SCENE ||
...
#else
            if (await FlowControl(code, _eventMapDataModel.eventId) == false)
#endif
            {
                // 選択肢
...
#else
        public override async Task ProcessCommandExecuteAsync() {
#endif
            if (_running == false)
            {
                MapEventExecutionController.Instance.RemoveEventQueue(ProcessCommandExecute);
...
#else
        public async Task EndAutoCommandChain() {
#endif
            ...
            //次イベント発動まで待ち　自動or並列実行されるイベントの際に、毎フレーム実行される可能性があるので、
            //実行時に、Waitを挟んで、CGさせるようにする 不具合対応(リークが発生する)UNITE_DEVELOPMENT-233
            MapEventExecutionController.Instance.SetWaitParallelAuto(0.1f);
        ...
        /// </summary>
        public static  bool IsInCustomMoveCommand(List<EventDataModel.EventCommand> eventCommands, int selectedIndex) {
        ...
        public string GetCustomMoveTargetEvent() {
            //カスタムイベントが入る可能性があるので、データ参照先を変更U292
            if (IsInCustomMoveCommand(_commandsInQueue, _currentCommandIndex) && _eventDataModel.eventCommands.Count > 0 && _eventDataModel.eventCommands[0].code == (int) EventEnum.EVENT_CODE_FLOW_CUSTOM_MOVE)
            {
                var parameters = _eventDataModel.eventCommands[0].parameters;
                return parameters[1];
            }
