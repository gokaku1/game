...
namespace RPGMaker.Codebase.Editor.MapEditor.Component.CommandEditor.GameProgress
{
    ...
    public class GameVariable : AbstractCommandEditor
    {
        ...
        public override void Invoke() {
            ...
            //グループから敵名を取得し設定する
            int TroopCount = GetBattleEnmyTroopCount();
            for (int i = 0; i < TroopCount; i++)
            {
                string Name = GetBattleEnemyName(i);
                enemyNameList.Add(Name);
                enemyIDList.Add(i.ToString());
                if (EventCommand.parameters[5] == i.ToString())
                {
                    selectID = i;
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        public int GetBattleEnmyTroopCount() 
        {
            int RetVal = 0;
            //バトルがサイドかフロントかを取得
            int ViewType = DataManager.Self().GetSystemDataModel().battleScene.viewType;

            //なにも配置していない場合も考慮して、初期数をあらかじめ決めておく
            {
                //未配置である場合は最大数を返す
                //フロントサイドの敵グループ上限数
                int FrontEnemyMax = 8;
            ...
            var ScenWindow = RPGMaker.Codebase.Editor.DatabaseEditor.DatabaseEditor.GetDatabaseSceneWindow();
            //エデッタ上でのバトルシーン表示時のみ
            if (ScenWindow.GetPreviewId() == DatabaseEditor.Window.SceneWindow.PreviewId.BattleScene)
            {
                if (DataManager.Self().GetGameTroop() != null)
                {
                    if (DataManager.Self().GetGameTroop().Members().Count >= 1)
                    {
                        RetVal = DataManager.Self().GetGameTroop().Members().Count;
                    }
                }
            }
            return RetVal;
        ...
        /// <returns></returns>
        public static string GetBattleEnemyName(int EnemyIndex) 
        {
            string stRet = "";
            stRet = "#" + (EnemyIndex + 1).ToString();
            ...
            if(ScenWindow.GetPreviewId() == DatabaseEditor.Window.SceneWindow.PreviewId.BattleScene)
            {
                if(DataManager.Self().GetGameTroop() != null)
                {
                    ...
                    {
                        // 敵データ取得
                        var EnemyData = DataManager.Self().GetGameTroop().Members()[EnemyIndex];
                        stRet = EnemyData.GetName();
                    }
                }
            }
            return stRet;
