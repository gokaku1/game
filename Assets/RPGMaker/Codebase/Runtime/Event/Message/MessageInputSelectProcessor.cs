...
#else
            ProcessAsync(eventId, command, eventCommands);
        }

        private async void ProcessAsync(string eventId, EventCommand command, List<EventCommand> eventCommands) {
#endif
...
#else
                _messageInputSelect = await HudDistributor.Instance.NowHudHandler().OpenInputSelectWindow();
#endif
