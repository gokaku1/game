...
namespace RPGMaker.Codebase.Editor.Common
{
    public static class WindowLayoutManager
    {
        public enum WindowLayoutId
        {
            ...
            MapEventCommandWindow1,              // 『イベントコマンド』枠 (選択)。┘重複   CommandWindowクラス。 ※MZではモーダルウィンドウ。
        ...
        private static readonly Dictionary<WindowLayoutId, Action<Type>> WindowOpenMethods = new()
            {
                ...
                {
                    WindowLayoutId.MapEventCommandWindow1,
                    dockTo => { OpenWindow<CommandWindowList>(WindowLayoutId.MapEventCommandWindow1, dockTo); }
                },
                {
                ...
                // map list
                {
                    WindowLayoutId.MapListWindow,
                    dockTo => { OpenWindow<MapListWindow>(WindowLayoutId.MapListWindow, dockTo); }
                },
                // event list
                {
                    WindowLayoutId.EventListWindow,
                    dockTo => { OpenWindow<EventListWindow>(WindowLayoutId.EventListWindow, dockTo); }
                },
                // EventDeleteWindow
                {
                    WindowLayoutId.EventDeleteWindow,
                    dockTo => { OpenWindow<HierarchyDeleteWindow>(WindowLayoutId.EventDeleteWindow, dockTo); }
                },
            };
        ...
        public static EditorWindow OpenAndDockWindow(
            ...
            Docker.DockTo(windowToDock, windowToOpen, dockPosition, childWeight);
