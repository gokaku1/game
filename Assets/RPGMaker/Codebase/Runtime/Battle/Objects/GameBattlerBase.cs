...
#else
        public virtual async Task AddParam(int paramId, int value) {
            _paramPlus[paramId] += value;
            await Refresh();
        }
#endif
...
#else
        public virtual async Task AddNewState(string stateId) {
#endif
...
#else
        public virtual async Task OnRestrict() {
            await UniteTask.Delay(0);
        }
#endif
...
#else
        public virtual async Task<bool> MeetsItemConditions(GameItem item) {
            return MeetsUsableItemConditions(item) && await DataManager.Self().GetGameParty().HasItem(item);
        }
#endif
...
#else
        public virtual async Task<bool> CanUse(GameItem item) {
#endif
            if (item == null)
            {
                return false;
...
#else
                return await MeetsItemConditions(item);
#endif
            }
            return false;
        }
...
#else
        public virtual async Task<bool> CanAttack(string skillId = AttackSkillId) {
            return await CanUse(new GameItem(skillId, GameItem.DataClassEnum.Skill));
        }
#endif
...
#else
        public virtual async Task<bool> CanGuard() {
            return await CanUse(new GameItem(GuardSkillId, GameItem.DataClassEnum.Skill));
        }
#endif
