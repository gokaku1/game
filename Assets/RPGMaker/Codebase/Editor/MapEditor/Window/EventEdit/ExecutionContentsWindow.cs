...
using SceneWindow = RPGMaker.Codebase.Editor.DatabaseEditor.Window.SceneWindow;
...
namespace RPGMaker.Codebase.Editor.MapEditor.Window.EventEdit
{
    ...
    public class ExecutionContentsWindow : BaseWindow
    {
        ...
        private static void InitializeOnLoad() {
            if (RPGMakerDefaultConfigSingleton.instance.EventComanedMod == 0)
            {
                //イベント編集中であった場合、閉じる
            ...
            {
                //イベント編集中であった場合、閉じる
                WindowLayoutManager.CloseWindow(WindowLayoutManager.WindowLayoutId.MapEventCommandWindow1);
            }
        }
        ...
        public void Init(
            ...
            if (initEvent && _eventDataModelEntity != null)
            {
        ...
        private void CreateCommandButton(EventCommand eventCommand, int index) {
            var btn = new Button();
            var element = _getEventCommandLabelTextUseCase.Invoke(eventCommand);
            btn.Add(element);
            var currentIndex = index;

            var ud = element.userData as AbstractEventText.FolderStateChangeDetectableUserData;
            if (ud != null)
            {
                ud.OnFolderStateChanged += (state) =>
                {
                    SetUpData();
                    RefreshCommandListDisplay();
                };
        ...
        private void ShowEventCommand(int selectedIndex = 0) {
            ...
            var eventEditWindow = (EventEditWindow)WindowLayoutManager.GetActiveWindow(
                WindowLayoutManager.WindowLayoutId.MapEventEditWindow);
            var sceneWindow = (SceneWindow)WindowLayoutManager.GetActiveWindow(
                WindowLayoutManager.WindowLayoutId.DatabaseSceneWindow);
            var eventEditCanvas = eventEditWindow?.EventEditCanvas;
            var previewSceneElement = sceneWindow?.PreviewSceneElement;
        ...
        /// ポストエフェクトプレビューを解除する
        /// </summary>
        private void ReleaseEffectPreview() {
            switch (_eventType)
            {
                case EventType.Battle:
                    var sceneWindow = (SceneWindow)WindowLayoutManager.GetActiveWindow(
                        WindowLayoutManager.WindowLayoutId.DatabaseSceneWindow);
                    var previewSceneElement = sceneWindow?.PreviewSceneElement;
                    previewSceneElement?.EffectRenderer?.SetEffectMaterial(null);
                    previewSceneElement?.EffectRenderer?.SetDirty(false);
                    sceneWindow.Render();
                    break;
                ...
                default:
                    var eventEditWindow = (EventEditWindow)WindowLayoutManager.GetActiveWindow(
                        WindowLayoutManager.WindowLayoutId.MapEventEditWindow);
                    var eventEditCanvas = eventEditWindow?.EventEditCanvas;
                    eventEditCanvas?.EffectRenderer?.SetEffectMaterial(null);
                    eventEditCanvas?.EffectRenderer?.SetDirty(false);
                    if (eventEditWindow != null && eventEditWindow.EventEditCanvas != null)
                    {
                        eventEditCanvas?.Refresh();
                    }
                    break;
            }
        }
        ...
        readonly Dictionary<int, List<int>> CommandIdEndCommandIdsDic = new Dictionary<int, List<int>>()
        {
            {(int) EventEnum.EVENT_CODE_FLOW_LOOP,        new List<int>(){ (int) EventEnum.EVENT_CODE_FLOW_LOOP_END } },
            {(int) EventEnum.EVENT_CODE_FLOW_CUSTOM_MOVE, new List<int>(){ (int) EventEnum.EVENT_CODE_FLOW_CUSTOM_MOVE_END } },
            {(int) EventEnum.EVENT_CODE_FLOW_IF,          new List<int>(){ (int) EventEnum.EVENT_CODE_FLOW_ELSE, (int) EventEnum.EVENT_CODE_FLOW_ENDIF } },
            {(int) EventEnum.EVENT_CODE_FLOW_ELSE,        new List<int>(){ (int) EventEnum.EVENT_CODE_FLOW_ENDIF } },
            {(int) EventEnum.EVENT_CODE_SCENE_SET_BATTLE_CONFIG,        new List<int>(){ (int) EventEnum.EVENT_CODE_SCENE_SET_BATTLE_CONFIG_END } },
            {(int) EventEnum.EVENT_CODE_SCENE_SET_BATTLE_CONFIG_WIN,    new List<int>(){ (int) EventEnum.EVENT_CODE_SCENE_SET_BATTLE_CONFIG_ESCAPE, (int)EventEnum.EVENT_CODE_SCENE_SET_BATTLE_CONFIG_LOSE, (int) EventEnum.EVENT_CODE_SCENE_SET_BATTLE_CONFIG_END } },
            {(int) EventEnum.EVENT_CODE_SCENE_SET_BATTLE_CONFIG_ESCAPE, new List<int>(){ (int) EventEnum.EVENT_CODE_SCENE_SET_BATTLE_CONFIG_LOSE, (int) EventEnum.EVENT_CODE_SCENE_SET_BATTLE_CONFIG_END } },
            {(int) EventEnum.EVENT_CODE_SCENE_SET_BATTLE_CONFIG_LOSE,   new List<int>(){ (int) EventEnum.EVENT_CODE_SCENE_SET_BATTLE_CONFIG_END } },
            {(int) EventEnum.EVENT_CODE_MESSAGE_INPUT_SELECT,           new List<int>(){ (int) EventEnum.EVENT_CODE_MESSAGE_INPUT_SELECT_END } },
            {(int) EventEnum.EVENT_CODE_MESSAGE_INPUT_SELECT_SELECTED,  new List<int>(){ (int) EventEnum.EVENT_CODE_MESSAGE_INPUT_SELECT_SELECTED, (int) EventEnum.EVENT_CODE_MESSAGE_INPUT_SELECT_CANCELED, (int) EventEnum.EVENT_CODE_MESSAGE_INPUT_SELECT_END } },
            {(int) EventEnum.EVENT_CODE_MESSAGE_INPUT_SELECT_CANCELED,  new List<int>(){ (int) EventEnum.EVENT_CODE_MESSAGE_INPUT_SELECT_END } },
        };
        readonly Dictionary<int, List<int>> CommandIdUpToCommandIdsDic = new Dictionary<int, List<int>>()
        {
            {(int) EventEnum.EVENT_CODE_MESSAGE_TEXT,          new List<int>(){ (int) EventEnum.EVENT_CODE_MESSAGE_TEXT_ONE_LINE } },
            {(int) EventEnum.EVENT_CODE_MESSAGE_TEXT_SCROLL,   new List<int>(){ (int) EventEnum.EVENT_CODE_MESSAGE_TEXT_SCROLL_ONE_LINE } },
            {(int) EventEnum.EVENT_CODE_SCENE_SET_SHOP_CONFIG, new List<int>(){ (int) EventEnum.EVENT_CODE_SCENE_SET_SHOP_CONFIG_LINE } },
            {(int) EventEnum.EVENT_CODE_ACTOR_CHANGE_NAME,     new List<int>(){ (int) EventEnum.EVENT_CODE_ACTOR_CHANGE_NICKNAME, (int) EventEnum.EVENT_CODE_ACTOR_CHANGE_PROFILE } },
            {(int) EventEnum.EVENT_CODE_BATTLE_CHANGE_STATUS,  new List<int>(){ (int) EventEnum.EVENT_CODE_BATTLE_CHANGE_MP, (int) EventEnum.EVENT_CODE_BATTLE_CHANGE_TP } },
        };
        /// <summary>
        /// コマンドの折りたたみ設定に合わせてイベントコマンドリストの表示状態を更新する。
        /// </summary>
        private void RefreshCommandListDisplay() {
            var eventCommands = _eventDataModelEntity.eventCommands;
            var children = _scrollView.contentContainer.Children();
            for (int i = 0; i < eventCommands.Count; i++)
            {
                var command = eventCommands[i];
                var child = children.ElementAt(i);
                child.style.display = DisplayStyle.Flex;
                if (command.code == (int) EventEnum.EVENT_CODE_SCENE_SET_BATTLE_CONFIG && command.parameters[2] == "0" && command.parameters[3] == "0")
                {
                    //逃走不可で敗北不可の場合は、EVENT_CODE_SCENE_SET_BATTLE_CONFIG_ENDの行が存在しない。
                    continue;
                }
                if (AbstractEventText.GetFolderParameter(command) == "0")
                {
                    int hideStart = -1;
                    int hideEnd = -1;
                    if (CommandIdEndCommandIdsDic.ContainsKey(command.code))
                    {
                        var endCommandIds = CommandIdEndCommandIdsDic[command.code];
                        hideStart = i + 1;
                        for (var j = hideStart; j < eventCommands.Count; j++)
                        {
                            if (endCommandIds.Contains(eventCommands[j].code) &&
                                eventCommands[j].indent == command.indent)
                            {
                                hideEnd = j - 1;
                                break;
                            }
                    ...
                    if (CommandIdUpToCommandIdsDic.ContainsKey(command.code))
                    {
                        var upToCommandIds = CommandIdUpToCommandIdsDic[command.code];
                        hideStart = i + 1;
                        for (var j = hideStart; j < eventCommands.Count; j++)
                        {
                            if (!upToCommandIds.Contains(eventCommands[j].code))
                            {
                                break;
                            }
                            hideEnd = j;
        ...
        private void CreateEventCommand(int selectedIndex) {
            ...
            CommandWindow commonWindow = null;
            if (RPGMakerDefaultConfigSingleton.instance.EventComanedMod == 0)
            {
                commonWindow = (CommandWindow) WindowLayoutManager.GetOrOpenWindow(
            ...
            if (EventCommandChainLauncher.IsInCustomMoveCommand(_eventDataModelEntity.eventCommands, selectedIndex))
            {
        ...
        private Dictionary<int, int> _ngCommandInfoForCustomMoveCommand = new Dictionary<int, int>()
        {
            ...
            {(int)EventEnum.EVENT_CODE_FLOW_JUMP_COMMON, (int)EventEnum.EVENT_CODE_FLOW_JUMP_COMMON},
        ...
        private (int, int) GetRelevanceLinesIndex(int index) {
            ...
            switch ((EventEnum) Enum.ToObject(typeof(EventEnum), currentCommand.code))
            {
                ...
                case EventEnum.EVENT_CODE_BATTLE_CHANGE_TP: // [敵キャラのステータス増減]のTP増減
                    ...
                    for (var i = index; i >= 0; i--)
                        //U348 参照先修正
                        if (_eventDataModelEntity.eventCommands[i].code == (int) EventEnum.EVENT_CODE_BATTLE_CHANGE_STATUS &&
                            _eventDataModelEntity.eventCommands[i].indent == currentCommand.indent)
                        {
                            tmpIndex = i;
                            headIndex = Math.Min(i, headIndex);
