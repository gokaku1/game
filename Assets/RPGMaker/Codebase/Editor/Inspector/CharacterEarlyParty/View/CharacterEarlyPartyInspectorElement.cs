...
namespace RPGMaker.Codebase.Editor.Inspector.CharacterEarlyParty.View
{
    ...
    public class CharacterEarlyPartyInspectorElement : AbstractInspectorElement
    {
        ...
        override protected void InitializeContents() {
            ...
            //復活ポイント設定
            var ListName = new List<string>() { EditorLocalize.LocalizeText("WORD_1068"), EditorLocalize.LocalizeText("WORD_0354")};
            //初期位置
            Foldout InitialPointFoldout = RootContainer.Query<Foldout>("foldout_2");
            //初期レベル
            Label RespwnPointLabel = new Label();
            RespwnPointLabel.text = EditorLocalize.LocalizeText("WORD_1653");
            var RespwnPointField = new PopupFieldBase<string>(ListName, _systemSettingDataModel.initialParty.respwnPointType);
            //RespwnPointField.
            RespwnPointField.RegisterValueChangedCallback(evt =>
            {
                int selectedIndex = RespwnPointField.index;
                _systemSettingDataModel.initialParty.respwnPointType = selectedIndex;
                if (_systemSettingDataModel.initialParty.respwnPointType >= 2)
                {
                    _systemSettingDataModel.initialParty.respwnPointType = 1;
                }
            });
            var RespwnPoint = RootContainer.Q<VisualElement>("RespwnPointField");
            RespwnPoint.Add(RespwnPointField);
        }
