...
namespace RPGMaker.Codebase.Editor.Hierarchy.Region.Map.View
{
    ...
    public class MapHierarchyView : AbstractHierarchyView
    {
        ...
        private CommonMapHierarchyView _commonEventHierarchyView;   //マップリストで選択状態のマップのイベントリスト管理用


        // UI要素
        //--------------------------------------------------------------------------------------------------------------
        MapListView _mapListView;   //マップリスト表示用
        EventListView _eventListView;   //イベントリスト表示用

        public MapListView GetMapListView() { return _mapListView; }

        public EventListView GetEventListView() { return _eventListView; }

        public CommonMapHierarchyView GetEventHierarchyView() {
            return _commonEventHierarchyView;
        }
        ...
        override protected void InitContentsData() {
            ...
            // マップリストはmapListFoldoutにぶら下がる。mapListFoldoutはヒエラルキー側ではなく、マップリストウィンドウ側に表示させたいので、MapListViewに引き渡す。
            //GetFoldout("mapSettingFoldout").Add(mapListFoldout);
            _mapListView = new MapListView(mapListFoldout, this);
            SetFoldout("mapListFoldout", mapListFoldout);

            // "イベントリスト"Foldout。イベントリストは、イベントリストウィンドウ側に表示させたいので、eventListFoldoutをEventListViewに引き渡す。
            var eventListFoldout = new Foldout { text = EditorLocalize.LocalizeText("WORD_1638") };
            eventListFoldout.name = "eventListFoldout";
            _eventListView = new EventListView(eventListFoldout, this);
            SetFoldout("eventListFoldout", eventListFoldout);
        ...
        protected override void SetParentContextMenu(Dictionary<string, List<string>> contextMenuDic) {
            foreach (var dic in contextMenuDic)
            {
                ...
                {
                    // 従来の「マップリスト」がなくなった代わりに、マップリストウィンドウの「menu」を使う。
                    //BaseClickHandler.ClickEvent(GetFoldout(keyName), evt =>
                    BaseClickHandler.ClickEvent(_mapListView.GetPopupArea(), evt =>
                    {
        ...
        protected override VisualElement DuplicateDataModel(string keyName, string uuId) {
            switch (keyName)
            {
                ...
                case KeyNameMapList:
                    ...
                    //U358 Indexの計算を変更
                    mapData.index = Editor.Hierarchy.Hierarchy.mapManagementService.GetNextNewMapIndex();
                    ...
                    //マップ内イベントもコピーする。
                    MapEditor.Component.Canvas.EventEditCanvas.CopyMapEvents(uuId, mapData.id, mapData.name);
        ...
        //<summary>
        //マップリストウィンドウ内のマップ名がクリックされたときに呼び出される。
        //対象のマップの編集の準備を行う。
        //</summary>
        public void MapClicked(VisualElement mapVe) {
            _mapListView.LaunchEditMode(mapVe.name);
        }

        //<summary>マップリストウィンドウで選択状態のマップのIDを返す。</summary>
        public string GetActiveMapId() {
            return GetFoldout("mapListFoldout").Q<Button>(null, new[] { "active" })?.name;
        }

        //<summary>指定mapIdのMapDataModelインスタンスを返す。</summary>
        public MapDataModel GetMapEntity(string mapId) {
            var index = _mapEntities.FindIndex(x => x.id == mapId);
            return (index >= 0) ? _mapEntities[index] : null;
        }

        //<summary>マップリストウィンドウで選択状態のマップのMapDataModelインスタンスを返す。</summary>
        public MapDataModel GetActiveMapEntity() {
            var mapId = GetActiveMapId();
            var index = _mapEntities.FindIndex(x => x.id == mapId);
            return (index >= 0) ? _mapEntities[index] : null;
        }

        //<summary>MapHierarchyInfoインスタンスを返す。</summary>
        public MapHierarchyInfo GetMapHierarchyInfo() {
        ...
        private void SetMap() {
            ...
            var lastActiveButton = GetFoldout("mapListFoldout").Q<Button>(null, new[] { "active" });
            ...
            //イベントリストはイベントリストウィンドウ内に表示するので、その管理用インスタンスを作成。
            var commonMap = new CommonMapHierarchyView();
            _commonEventHierarchyView = commonMap.AddEventVisualElement(mapHierarchyInfo, this, GetFoldout("eventListFoldout"));
            //マップリスト更新前に選択していたマップを再選択。
            //InvokeSelectableElementAction();
            if (lastActiveButton != null)
            {
                Editor.Hierarchy.Hierarchy.InvokeSelectableElementAction(lastActiveButton);
            }
        }
        ...
        private void UpdateMap() {
            ...
            switch (data[(int)RefreshType.Type])
            {
                ...
                case RefreshTypeMapDuplicate:
                    ...
                    Editor.Hierarchy.Hierarchy.InvokeSelectableElementAction(commonMap.MapFoldout);
                    _mapHierarchy.View.MapClicked(commonMap.MapFoldout);
                    break;
                //マップの削除
                ...
                case RefreshTypeMapSize:
                    for (int i = 0; i < _commonMapHierarchyViews.Count; i++)
                    {
                        if (_commonMapHierarchyViews[i].MapId == data[(int) RefreshType.MapId])
                        {
                            ...
                            (_commonMapHierarchyViews[i].MapFoldout as Button).text = mapEntity.name;
                            break;
                        }
                    }
                    break;
                ...
                case RefreshTypeEventName:
                    // 選択状態のマップのイベントリストを作成。
                    _mapHierarchy.View.GetEventHierarchyView().CreateEventContent(mapHierarchyInfo);
                    break;
