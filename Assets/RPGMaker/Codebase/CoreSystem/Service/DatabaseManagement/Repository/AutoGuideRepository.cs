...
#else
        public new async Task<List<AutoGuideDataModel>> Load() {
#endif
            if (DataModels != null)
            {
                // キャッシュがあればそれを返す
...
#else
            DataModels = (await ScriptableObjectOperator.GetClass<AutoGuideDataModel>(JsonPath)) as List<AutoGuideDataModel>;
#endif
            SetSerialNumbers();
            return DataModels;
        }
