...
namespace RPGMaker.Codebase.Runtime.Map
{
    ...
    public class MapEventExecutionController {
        ...
        /// <summary>
        /// 自動or並列イベントの終了から、次のイベント発動までの待ち状態かどうか
        /// </summary>
        private float _waitTimeParallelAuto = 0.0f;
        ...
        private EventOnMap _commonEventWithPicture = null;
...
#else
        public async Task InitEvents(MapDataModel mapDataModel, GameObject rootGameObject) {
#endif
...
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
                await _customMoveEventManager.InitForConstructor();
#endif
            } else
            {
...
#else
            await SetEventsOnMap(rootGameObject);
            foreach (var eventOnMap in _eventsOnMap) await eventOnMap.SetUp(mapDataModel);
#endif
        }
...
#else
        public async Task SetEventsOnMap(GameObject rootGameObject) {
#endif
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
            _mapEventEntities = _eventManagementService.LoadEventMapEntitiesByMapId(_mapDataModel.id);
#else
            _mapEventEntities = await _eventManagementService.LoadEventMapEntitiesByMapId(_mapDataModel.id);
#endif
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
...
#else
            _eventsOnMap = new List<EventOnMap>();
            foreach (var mapEvent in _mapEventEntities.Where(mapEvent => mapEvent.pages.Count > 0))
            {
                var positionOnTile = new Vector2(mapEvent.x, mapEvent.y);
                var evt = await _eventManagementService.LoadEventById(mapEvent.eventId);
                var eventOnMap = new GameObject().AddComponent<EventOnMap>();
                eventOnMap.gameObject.SetActive(false);
                await eventOnMap.Init(positionOnTile, mapEvent, evt);
                eventOnMap.gameObject.transform.SetParent(rootGameObject.transform);
...
#else
            List<EventCommonDataModel> eventCommonDataModels = await _eventManagementService.LoadEventCommon();
#endif
...
#else
                    var evt = await _eventManagementService.LoadEventById(data.eventId);
#endif
...
#else
                    await eventOnMap.Init(positionOnTile, eventMapDataModel, evt);
#endif
                    eventOnMap.gameObject.transform.SetParent(rootGameObject.transform);
            ...
            {
                eventOnMap.gameObject.SetActive(true);
            }
#endif
...
#else
                await _carryOverEventOnMap.SetToPositionOnTile(new Vector2(9999999.9f, 99999999.9f));
#endif
        ...
        public void RemoveCarryEventOnMap() {
            ...
            if (_autoExecuteEventsOnMap == _carryOverEventOnMap)
                _autoExecuteEventsOnMap = null;
            //U377 イベント登録も削除する
            _eventsOnMap.Remove(_carryOverEventOnMap);
            _carryOverEventOnMap = null;
        }
        ...
        /// 並列実行の待ち時間の初期化
        /// </summary>
        public void SetWaitParallelAuto(float time = 0.5f) 
        {
            if (_waitTimeParallelAuto < time)
                _waitTimeParallelAuto = time;
        }

        /// <summary>
        /// 並列&自動実行イベントの待ち時間が経過しているかをチェックする
        /// </summary>
        /// <returns></returns>
        private bool ChecParallelAutokWaitTime() 
        {
            bool bRet = false;
            //次イベント実行待ち状態の場合は処理しない
            if (_waitTimeParallelAuto > 0.0f)
            {
                _waitTimeParallelAuto -= Time.deltaTime;
                if (_waitTimeParallelAuto < 0.0f)
                {
                    _waitTimeParallelAuto = 0.0f;
                }
                else
                {
                    bRet = true;
...
#else
        public async Task UpdateEventWatch(Vector2 targetPositionOnTile) {
#endif
            ...
            if (_waitTimeParallelAuto < Time.deltaTime * 2) {
                InputHandler.UpdateWaitInputSystemState();
            }
            InputHandler.SwitchInputSystemState(false); //ウェイトで待たされるイベント用の入力に切り替え。

            // 自動実行イベント
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
            TryToAutoEvent(targetPositionOnTile);
#else
            await TryToAutoEvent(targetPositionOnTile);
#endif
...
#else
            await TryToParallelEvent(targetPositionOnTile);
#endif

            InputHandler.SwitchInputSystemState(true); //通常イベント用の入力に戻す。
...
#else
                if (_eventsOnMap[i] != null) await _eventsOnMap[i].UpdateCommonEvents();
#endif
            }
        }
...
#else
        public async Task TryToAutoEventMoveEnd(Vector2 targetPositionOnTile) {
#endif
            // ページ番号の切り替え
            for (int i = 0; i < _eventsOnMap.Count; i++)
            {
...
#else
                await _eventsOnMap[i].UpdateTimeHandlerAsync();
#endif
            }
...
#else
        public async Task TryToTalkToEvent(Vector2 targetPositionOnTile, Vector2 counterOverTargetPositionOnTile, TilesOnThePosition tilesOnThePosition) {
#endif
            //次イベント実行待ち状態の場合は処理しない
            if (_waitTime > 0.0f)
            {
...
#else
                        await tilesOnThePosition.InitForRuntime(MapManager.CurrentMapDataModel, targetPositionOnTile);
#endif
...
#else
        public async Task EndTriggerEvent(EventMapDataModel eventMapDataModel, EventDataModel eventDataModel) {
#endif
            //接触またはアクションキーイベント終了
...
#else
                    await _executeEventsOnMap.RestartMove();
#endif
...
#else
        public async Task TryToAutoEvent(Vector2 targetPositionOnTile) {
#endif
            if (_eventsOnMap == null)
                return;
            ...
            if (ChecParallelAutokWaitTime() == true) return;
...
#else
                if (await _eventsOnMap[i].CheckValid() == false) continue;
#endif
...
#else
        public async Task TryToTraitsToCommonEvent(EventMapDataModel mapDataModelEvent, EventDataModel eventDataModel) {
#endif
            //次イベント実行待ち状態の場合は処理しない
            if (_waitTime > 0.0f)
            {
...
#else
            await targetEventOnMap.Init(new Vector2(9999, 9999), mapDataModelEvent,eventDataModel);
#endif
            
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
            MenuManager.MenuBase.AllClose();
#else
            await MenuManager.MenuBase.AllClose();
#endif
        ...
        /// <param name="mapDataModelEvent"></param>
        /// <param name="eventDataModel"></param>
        public void ExecuteCommonEventWithPicture(EventMapDataModel mapDataModelEvent, EventDataModel eventDataModel)
        {
            //次イベント実行待ち状態の場合は処理しない
            if (_waitTime > 0.0f || _executeEventsOnMap != null) return;

            // コモンイベント用のオブジェクトを作成。処理が終わったら破棄する。
            _commonEventWithPicture = new GameObject().AddComponent<EventOnMap>();

            _eventsOnMap.Add(_commonEventWithPicture);
            _commonEventWithPicture.Init(new Vector2(9999, 9999), mapDataModelEvent, eventDataModel);

            //イベント実行
            _executeEventsOnMap = _commonEventWithPicture;
            if (_commonEventWithPicture.ExecuteCommonEvent(EndTriggerEvent, false) == false)
                _executeEventsOnMap = null;

            // 実行後イベント削除
            _eventsOnMap.Remove(_commonEventWithPicture);
            UnityEngine.Object.DestroyImmediate(_commonEventWithPicture.gameObject);
            _commonEventWithPicture = null;
...
#else
        public async Task EndAutoEvent(EventMapDataModel eventMapDataModel, EventDataModel eventDataModel) {
#endif
            //自動実行イベント終了
...
#else
                    await _autoExecuteEventsOnMap.RestartMove();
#endif
...
#else
        private async Task TryToParallelEvent(Vector2 targetPositionOnTile) {
#endif
            if (_eventsOnMap == null)
                return;
            //次イベント実行待ち状態の場合は処理しない
...
#else
        public async Task EndParallelEvent(EventMapDataModel eventMapDataModel, EventDataModel eventDataModel) {
#endif
            //終了して良ければ、配列から除去
            bool flg = false;
            int num = -1;
...
#else
                if (await _eventsOnMap[i].CheckValid() == false) break;
#endif
