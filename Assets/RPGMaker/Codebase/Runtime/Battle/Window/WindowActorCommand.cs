    ...
    public class WindowActorCommand : WindowCommand
    {
        ...
        public override async Task MakeCommandList() {
            ...
            if (_actor != null)
            {
                ...
                AddAttackCommand();
#else
                await AddAttackCommand();
#endif
                AddSkillCommands();
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                AddGuardCommand();
#else
                await AddGuardCommand();
#endif
                AddItemCommand();
            }
        }
...
#else
        public async Task AddAttackCommand() {
#endif
...
#else
            var inputtingAction = await BattleManager.InputtingAction();
            if (inputtingAction != null) inputtingAction.SetAttackSkill(skillId);
            AddCommand(TextManager.attack, "attack", await _actor.CanAttack(skillId));
#endif
...
#else
        public async Task AddGuardCommand() {
#endif
...
#else
            AddCommand(TextManager.guard, "defence", await _actor.CanGuard());
#endif
...
#else
        public async Task Setup(GameActor actor) {
#endif
            // ボタンを選択状態にする
...
#else
        public async Task SetBattlePreviewMode(GameActor actor) {
#endif
            _actor = actor;
            ClearCommandList();
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
