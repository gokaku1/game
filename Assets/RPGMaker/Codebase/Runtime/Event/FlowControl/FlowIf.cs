...
namespace RPGMaker.Codebase.Runtime.Event.FlowControl
{
    ...
    {
        /*
            分岐処理のために、クラスにしているが、クラスで、インスタンスで処理するの意味が不明で、
            ヒープを消費するので、static 関数に変更(メモリリーク対策用) by Mizo
        */

#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
        public static bool FrowIf(string eventId, EventDataModel.EventCommand command) 
#else
        public static async Task<bool> FrowIf(string eventId, EventDataModel.EventCommand command) 
#endif
        {
...
#else
            var sw = (await dm.LoadFlags()).switches;
            var vari = (await dm.LoadFlags()).variables;
            var actor = await dm.LoadCharacterActor();
            var classes = await dm.LoadCharacterActorClass();
#endif
            ...
            if (command.parameters[19] == "1")
            {
                ...
                if (actorData == null)
                {
                    ...
                    var actorId = command.parameters[20];
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                    actorData = partyChange.SetActorData(actorId,false);
#else
                    actorData = await partyChange.SetActorData(actorId,false);
#endif
                }
...
#else
                            if (partyActors[i].ActorId == actorData.actorId)
#endif
                            {
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
...
#else
                                for (var j = 0; j < partyActors[i].States.Count; j++)
                                    if (partyActors[i].States[j].id == command.parameters[27])
#endif
                                    {
                                        if (flowType != 1)
                                            return true;
...
#else
                        var enemy = members.ElementAtOrDefault(memberIndex);
#endif
            ...
            if (command.parameters[48] == "1")
            {
                ...
                {
                    case "0":
                        handleType = HandleType.Decide;
                        break;
                    case "1":
                        handleType = HandleType.Back;
                        break;
                    case "2":
                        handleType = HandleType.LeftShiftDown;
                        break;
                    case "3":
                    ...
                    case "6":
                        ...
                        break;
                    case "7":
                        handleType = HandleType.PageLeft;
                        break;
                    case "8":
                        handleType = HandleType.PageRight;
                        break;
                    case "9":
                        handleType = HandleType.RightClick;
                        break;
                }
                ...
                if (InputHandler.GetHandleState(handleType, inputType))
                {
                    if (flowType != 1)
                        return true;
                }
