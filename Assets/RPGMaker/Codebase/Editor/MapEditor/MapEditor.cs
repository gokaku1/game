...
namespace RPGMaker.Codebase.Editor.MapEditor
{
    public static class MapEditor
    {
        ...
        /// </summary>
        /// <param name="mapDataModel"></param>
        /// <param name="mapEventList"></param>
        public static void RemoveMap(MapDataModel mapDataModel, List<EventMapDataModel> mapEventList) {
            _mapManagementService.RemoveMap(mapDataModel);
            mapEventList.ForEach(mapEvent => DeleteEventMap(mapEvent));
            // 削除対象のマップ内に初期配置のイベントがあった際に削除する
            // プレイヤーの初期位置
            var updatePLoadSystem = _databaseManagementService.LoadSystem();
            if (updatePLoadSystem.initialParty.startMap.mapId == mapDataModel.id) {
                updatePLoadSystem.initialParty.startMap.mapId = "";
                updatePLoadSystem.initialParty.startMap.position[0] = 0;
                updatePLoadSystem.initialParty.startMap.position[1] = 0;
                _databaseManagementService.SaveSystem(updatePLoadSystem);
            }

            // 乗り物の初期位置
            var saveFlg = false;
            var updateVehicles = _databaseManagementService.LoadCharacterVehicles();
        ...
        /// <summary>
        /// ページの削除
        /// ページを連続する場合は PageListを昇順でソースとする必要がある…
        /// </summary>
        /// <param name="eventMapDataModel"></param>
        /// <param name="page"></param>
        public static void DeleteMultPage(EventMapDataModel eventMapDataModel, List<int> PageList) 
        {
            // 対象のEventDataModelをセーブファイルから削除。
            var eventManagementService = new EventManagementService();
            var eventList = eventManagementService.LoadEventsById(eventMapDataModel.eventId);
            ...
            foreach (var page in PageList)
            {
                ...
                foreach (var eventData in eventList)
                {
                    if (page == eventData.page)
                    { 
                        eventManagementService.DeleteEvent2(eventData);
                        break;
                    }
                }
                // 対象のページを削除。
            ...
            for (int i = 0; i < eventMapDataModel.pages.Count; i++)
            {
                eventMapDataModel.pages[i].page = i;
            }
            ...
            foreach (var eventData in eventList)
            {
                if (PageList.Contains(eventData.page) == false)
                {
                    if (eventData.page != PageNo)
                    {
                        eventManagementService.PageRepair2(eventData, PageNo);
                    }
                    PageNo++;
                }
            }
        ...
        public static string GetEventId() 
        {
            ...
            if (_eventMapDataModelEntity != null)
            {
                stEventID = _eventMapDataModelEntity.eventId;
            }
            return stEventID;
        }
        /// <summary>
        ...
        //イベントエデッタを更新
        public static void EventEditWindowRefresh() {
