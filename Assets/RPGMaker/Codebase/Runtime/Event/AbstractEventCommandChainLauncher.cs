...
using RPGMaker.Codebase.Runtime.Event.PostEffect;
    ...
    public abstract class AbstractEventCommandChainLauncher
    {
        ...
        protected readonly Dictionary<EventEnum, AbstractEventCommandProcessor> _commandProcessors =
            new Dictionary<EventEnum, AbstractEventCommandProcessor>
            {
                ...
                {EventEnum.EVENT_CODE_MOVE_RESPAWN_POINT, new RespawnPointProcessor()},
                ...
                {EventEnum.EVENT_CODE_MAP_CHANGE_MINIMAP, new MapChangeMinimapProcessor()},
                ...
                {EventEnum.EVENT_CODE_SCENE_CALL_UNITY_SCENE, new CallUnitySceneProcessor()},
                ...
                // ポストエフェクト
                {EventEnum.EVENT_CODE_POST_EFFECT_GLITCH, new PostEffectGlitchProcessor()},
                {EventEnum.EVENT_CODE_POST_EFFECT_BLUR, new PostEffectBlurProcessor()},
                {EventEnum.EVENT_CODE_POST_EFFECT_BLOOM, new PostEffectBloomProcessor()},
                {EventEnum.EVENT_CODE_POST_EFFECT_FILL, new PostEffectFillProcessor()},
                {EventEnum.EVENT_CODE_POST_EFFECT_PIXELATE, new PostEffectPixelateProcessor()},
                {EventEnum.EVENT_CODE_POST_EFFECT_REMOVE, new PostEffectRemoveProcessor()}
            };
...
#else
        public void ProcessCommand(string eventId) {
            _ = ProcessCommandAsync(eventId);
        }
        public virtual async Task ProcessCommandAsync(string eventId) {
...
#else
        public void ProcessCommandExecute() {
            _ = ProcessCommandExecuteAsync();
        }
        public virtual async Task ProcessCommandExecuteAsync() {
            await UniteTask.Delay(0);
#endif
        }
...
#else
        protected async Task<bool> FlowControl(EventEnum code, string eventId) {
#endif
            ...
            switch (code)
            {
                case EventEnum.EVENT_CODE_FLOW_IF:
                    ...
                    //if判定で、なぜかnew FlowIfで、クラス生成して、判定していたので、
                    //クラス生成しないで、判定だけする関数に変更(UNITE_DEVELOPMENT-233)

#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                    if (FlowIf.FrowIf(eventId, _commandsInQueue[_currentCommandIndex]))
#else
                    if (await FlowIf.FrowIf(eventId, _commandsInQueue[_currentCommandIndex]))
#endif
                    {
                        // そのまま次へ
                    }
...
#else
                    await MapEventExecutionController.Instance.GetCustomMoveEventManager().RegisterCustomMove(_commandEventID, _commandsInQueue, _currentCommandIndex, endIndex, parallelProcess);
#endif
                    if (!parallelProcess)
                    {
                        // 完了までウェイト。
...
#else
                        var eventData = (await _eventManagementService.LoadEventById(_commandsInQueue[_currentCommandIndex].parameters[0])).DataClone<EventDataModel>();
#endif
                        // Indent設定する
