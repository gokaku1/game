...
#else
            var animationDataModels = await _databaseManagementService.LoadAnimation();
#endif
...
#else
        private async Task InitForBattle() {
#endif
            if (_prefab != null)
                return;
...
#else
            var animation = await _databaseManagementService.LoadAnimation();
#endif
...
#else
            await EffectSet(_animationDataModel.particleId);
#endif
...
#else
        public async Task PlayAnimationForBattle(
#endif
            Action endAction,
            Action closeAction,
            string animationDataId,
...
#else
            await EffectSet(_animationDataModel.particleId, true);
#endif
...
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
            if (!isEffekseer && _animationPrefab == null) return;
#endif
            // モードが変わった際に破棄
...
#else
 ((await ScriptableObjectOperator.GetClass<AssetManageDataModel>("SO/" + id + ".asset")) as AssetManageDataModel).imageSettings[0].path;
#endif
#endif
            ...
            //U323 バトルエフェクトのOffsetを設定する
            var Offset = new Vector3(float.Parse(offsetArr[0]), float.Parse(offsetArr[1]));
            if (Offset != Vector3.zero)
            {
                pos += Vector3.Scale(Offset,transform.lossyScale);
            }
            ...
            if (effekseer)
            {
...
#else
                    await UnityEditorWrapper.AssetDatabaseWrapper.LoadAssetAtPath<EffekseerEffectAsset>(
#endif
                ...
                if (!Commons.IsURP())
                {
                    _effekseerHandle.layer = LayerMask.NameToLayer("IgnorePostEffect");
                }
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
                isEffekseer = true;
...
#else
                    await UnityEditorWrapper.AssetDatabaseWrapper.LoadAssetAtPath<GameObject>(PathManager.ANIMATION_PREFAB + path);
#endif
