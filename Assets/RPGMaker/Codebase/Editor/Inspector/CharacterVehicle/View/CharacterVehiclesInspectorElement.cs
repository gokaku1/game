...
namespace RPGMaker.Codebase.Editor.Inspector.CharacterVehicle.View
{
    ...
    public class CharacterVehiclesInspectorElement : AbstractInspectorElement
    {
        ...
        override protected void InitializeContents() {
            ...
            //シーンからオーディオリスナーを取得する
            audioSource = SoundHelper.GetEditorSceneAudioSource();
            ...
            //表示レイヤーの指定
            VisualElement selectLayeDropdown = RootContainer.Query<VisualElement>("Layer_button");
            List<string> LayeNameList = new List<string>() { "A","B","C","D",EditorLocalize.LocalizeText("WORD_0871"), EditorLocalize.LocalizeText("WORD_0872")};
            var selecttLayeDropdownChoices = LayeNameList;
            var selectLayeDropdownPopupField =
            new PopupFieldBase<string>(selecttLayeDropdownChoices, (int)_vehiclesData.layerType);
            selectLayeDropdown.Add(selectLayeDropdownPopupField);
            selectLayeDropdownPopupField.RegisterValueChangedCallback(evt =>
            {
                _vehiclesData.layerType = (VehiclesDataModel.LayerType) selectLayeDropdownPopupField.index;
                Save();
            });
            ...
            memoText.value = _vehiclesData.memo;
            memoText.RegisterCallback<FocusOutEvent>(evt =>
            {
                _vehiclesData.memo = memoText.value;
                Save();
            });
            ...
            {
                var selectModalWindow = 
                    AssetPicker.Instantiate(true, "WORD_0113", true);
                selectModalWindow.CharacterSdType = AssetPicker.CharacterType.Map;
