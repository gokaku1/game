...
namespace RPGMaker.Codebase.Runtime.Shop
{
    public class ItemShop : WindowBase
    {
...
#else
            _items = await _databaseManagementService.LoadItem();
            _weapons = await _databaseManagementService.LoadWeapon();
            _armors = await _databaseManagementService.LoadArmor();
#endif
        ...
        async Task ItemShopWindow() {
            ...
            SetupBelongingList();
#else
            await SetupSaleItemList();
            await SetupBelongingList();
#endif
...
#else
        private async Task SetupSaleItemList() {
#endif
...
#else
                        await CreateContent(i, contentParent[0], ShopState.BUY, ItemShopContent.Type.ITEM,
#endif
                            salesItemDataList[i])
                    );
            }
...
#else
                        await CreateContent(i, contentParent[0], ShopState.BUY, ItemShopContent.Type.WEAPON,
#endif
...
#else
                        await CreateContent(i, contentParent[0], ShopState.BUY, ItemShopContent.Type.ARMOR,
#endif
...
#else
        private async Task SetupBelongingList() {
#endif
...
#else
                            await CreateContent(i, contentParent[4], ShopState.IMPORTANT, ItemShopContent.Type.IMPORTANT,
#endif
                                item)
                        );
...
#else
                            await CreateContent(i, contentParent[1], ShopState.ITEM, ItemShopContent.Type.ITEM, item)
#endif
                        );
                }
            }
...
#else
                        await CreateContent(i, contentParent[2], ShopState.WEAPON, ItemShopContent.Type.WEAPON, weapon)
#endif
                    );
            }
...
#else
                        await CreateContent(i, contentParent[3], ShopState.ARMOR, ItemShopContent.Type.ARMOR, armor)
#endif
                    );
            }
...
#else
        async Task<ItemShopContent> CreateContent<T>(
#endif
            int i,
            Transform parent,
            ShopState kind,
...
#else
            await ItemDataSetting(kind, itemContent, i, myKind, itemData);
#endif
            return itemContent;
        }
...
#else
                            await itemContent.SetText(itemBasic.name, itemBasic.price.ToString(), itemBasic.iconId);
#endif
                            break;
...
#else
                            await itemContent.SetText(weaponBasic.name, weaponBasic.price.ToString(), weaponBasic.iconId);
#endif
                            break;
...
#else
                            await itemContent.SetText(armorBasic.name, armorBasic.price.ToString(), armorBasic.iconId);
#endif
                            break;
                    }
...
#else
                    await itemContent.SetText(itemBasic.name, itemContent.ItemCount.ToString(), itemBasic.iconId);
#endif
...
#else
                    await itemContent.SetText(weaponBasic.name, itemContent.ItemCount.ToString(), weaponBasic.iconId);
#endif
                    //売却不可のチェックが入っている場合には、売却できない
...
#else
                    await itemContent.SetText(armorBasic.name, itemContent.ItemCount.ToString(), armorBasic.iconId);
#endif
                    //売却不可のチェックが入っている場合には、売却できない
...
#else
                    await itemContent.SetText(itemBasic.name, itemContent.ItemCount.ToString(), itemBasic.iconId, itemBasic.itemType);
#endif
                    break;
            }
        }
...
#else
                        _ = _activeTradeContent.SetTradeItemInfo(itemBasic.iconId, itemBasic.name, _tradeNum, itemBasic.price);
#endif
                    else
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                        _activeTradeContent.SetTradeItemInfo(itemBasic.iconId, itemBasic.name, _tradeNum, itemBasic.sell);
#else
                        _ = _activeTradeContent.SetTradeItemInfo(itemBasic.iconId, itemBasic.name, _tradeNum, itemBasic.sell);
#endif
                    break;
                case ItemShopContent.Type.WEAPON:
...
#else
                        _ = _activeTradeContent.SetTradeItemInfo(weaponBasic.iconId, weaponBasic.name, _tradeNum, weaponBasic.price);
#endif
                    else
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                        _activeTradeContent.SetTradeItemInfo(weaponBasic.iconId, weaponBasic.name, _tradeNum, weaponBasic.sell);
#else
                        _ = _activeTradeContent.SetTradeItemInfo(weaponBasic.iconId, weaponBasic.name, _tradeNum, weaponBasic.sell);
#endif
                    break;
                case ItemShopContent.Type.ARMOR:
...
#else
                        _ = _activeTradeContent.SetTradeItemInfo(armorBasic.iconId, armorBasic.name, _tradeNum, armorBasic.price);
#endif
                    else
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                        _activeTradeContent.SetTradeItemInfo(armorBasic.iconId, armorBasic.name, _tradeNum, armorBasic.sell);
#else
                        _ = _activeTradeContent.SetTradeItemInfo(armorBasic.iconId, armorBasic.name, _tradeNum, armorBasic.sell);
#endif
                    break;
            }
        }
        ...
        public async Task BuyAndSellAsync() {
#endif
            if (_selectedItemContent == null) return;
...
#else
                            await _selectedItemContent.SetText(dataModel.name, _selectedItemContent.ItemCount.ToString(),
#endif
                                dataModel.iconId);
                            panelList[4].transform.Find("UsedItemList").Find("Num").GetComponent<Text>().text =
