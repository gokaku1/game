...
namespace RPGMaker.Codebase.Runtime.Battle.Sprites
{
    ...
    public class SpritesetBattle : MonoBehaviour
    {
        ...
        /// サイドビュー時の初期位置保存用
        /// </summary>
        private Dictionary<int,List<Vector2>> _sideObjectOrgPos;
...
#else
        public async Task RenderBattleScenePreview(string troopId) {
#endif
            ...
            try
            {
                ...
                await DataManager.Self().SetTroopForBattle(gameTroop, true);
#endif
...
#else
                        _battleWindowObj = await new WindowInitialize().Create(gameObject);
#endif
...
#else
                    await actorCommandWindow.Initialize();
                    await actorCommandWindow.SetBattlePreviewMode((await DataManager.Self().GetGameParty().GetActors())[0]);
                    await actorCommandWindow.Show();
#endif
                ...
                InitialPositionObjInit();
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                CreateBattleback();
                CreateEnemies();
#else
                await CreateBattleback();
                await CreateEnemies();
#endif
...
#else
                    await _actorSprites[i].SetEndEntryMotionPreview();
#endif
                }
            }
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
...
#else
        public async Task CreateBattleback() {
#endif
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
...
#else
            _back1Sprite.sprite = await ImageManager.LoadBattleback1(Battleback1Name());
            _back2Sprite.sprite = await ImageManager.LoadBattleback2(Battleback2Name());
#endif
...
#else
        public async Task CreateEnemies() {
#endif
            if (originEnemy == null)
                return;
...
#else
            var enemies = await DataManager.Self().GetGameTroop().Members();
#endif
...
#else
                    await enemySprite.Initialize(enemy);
                    await enemySprite.UpdateBitmap();
#endif
...
#else
        public async Task TransformEnemy(GameEnemy gameEnemy) {
#endif
            ...
            if (_systemdata.battleScene.viewType == 1)
            {
...
#else
                TroopDataModel.SideViewMember sideViewMember = troopData.sideViewMembers[await gameEnemy.Index()]; ;
#endif
                ...
                for (var i3 = 0; i3 < _enemySprites.Count; i3++)
                {
                    ...
                    {
                        //U330 同一位置か?　位置違い敵を参照してしまうため修正
                        if (gameEnemy.Index() == _enemySprites[i3].GetGameEnemy().Index())
                        {
                ...
                //上揃え、下揃えの適用
                // U329 初期位置を取得する
                Vector2 position = GetPosition(sideViewMember.position1, sideViewMember.position2, enemyDataWork.images.autofit, enemyDataWork.images.autofitPattern);
                //U329　初期位置に戻す
                enemySprite.transform.parent.GetComponent<RectTransform>().anchoredPosition = position;
                ...
                //傾斜させる
                sideEnemyInclined(sideViewMember.position1,_systemdata.battleScene.sideEnemyInclined, enemyDataWork.images.autofitPattern, enemySprite);
            }
            else
            {
                //敵一覧
...
#else
        public async Task CreateActors(bool battlePreview = false) {
#endif
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
...
#else
                    await actorSprite.Initialize(actor);
#endif
...
#else
                        await _actorSprites[i].SetActorHome(num++);
#endif
                    }
...
#else
                            await actorSprite.SetEndEntryMotion();
#endif
                            break;
                        }
                }
...
#else
        public async Task UpdateActorsPosition() {
#endif
            //サイドビューの場合SDキャラクターの表示を行う
            if (_systemdata.battleScene.viewType == 1)
            {
        ...
        /// <summary>
        /// サイドビュー時の初期位置を保存する　U329
        /// </summary>
        private void InitialPositionObjInit() 
        {
            _sideObjectOrgPos = new Dictionary<int, List<Vector2>>();
            int Index = 0;
            //サイド時の位置を保存する
            Transform children = _sideObject.GetComponentInChildren<Transform>();
            foreach (Transform ob in children)
            {
                Transform ChildrenObj = ob.GetComponentInChildren<Transform>();
                List < Vector2 > ListPos = new List<Vector2>();
                foreach (Transform ObjChild in ChildrenObj)
                {
                    var Pos = ObjChild.transform.GetComponent<RectTransform>().anchoredPosition;
                    ListPos.Add(Pos);
                }
                _sideObjectOrgPos.Add(Index, ListPos);
                Index++;
            }
        }
        /// <summary>
        ...
        /// <param name="autofitPattern"></param>
        /// <returns></returns>
        private Vector2 GetPosition(int pos1, int pos2, int autofit, int autofitPattern)
        {
            ...
            RetPos = _sideObjectOrgPos[p1][p2];
        ...
        /// </summary>
        /// <param name="inclined"></param>
        private void sideEnemyInclined(int pos1,int inclined , int autofitPattern , SpriteEnemy enemySprite) 
        {
            //前衛、後衛
            var p1 = pos1 == 0 ? 0 : 2;
            //中型またはボス型の場合
            ...
            float offset = _offsetSmallX;
            if (p1 == 1 || p1 == 3 )
            {
                //中型or大型
                offset = _offsetLargeX;
            }

            if (enemySprite.transform.parent.parent.name == "Items" + p1)
            {
                Vector2 pos = Vector2.zero;
                pos = transform.GetComponent<RectTransform>().anchoredPosition;
                if (enemySprite.transform.parent.name == "Items0")
                {
                    //上の敵
                    pos = new Vector2(pos.x + offset * inclined, 0);
                }
                else
                if (enemySprite.transform.parent.name == "Items4")
                {
                    //下の敵
                    pos = new Vector2(pos.x - offset * inclined, 0);
                }
                enemySprite.transform.parent.GetComponent<RectTransform>().anchoredPosition += pos;
            }
        }
