...
#else
        public async Task EquipsTypeDisplay(RuntimeActorDataModel runtimeActorDataModel, EquipMenu equipMenu) {
#endif
            //初期化処理
...
#else
                await equipmentItems.NowEquips(equipType, runtimeActorDataModel, equipMenu, i);
#endif
...
#else
                    if (await ItemManager.CheckTraitEquipLock(runtimeActorDataModel, equipType) ||
                        await ItemManager.CheckTraitEquipSea(runtimeActorDataModel, equipType, i))
#endif
                    {
...
#else
                        if (await ItemManager.CheckTraitEquipSea(runtimeActorDataModel, equipType, i))
#endif
                        {
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                            equipmentItems.NowEquips(equipType, runtimeActorDataModel, equipMenu, i);
...
#else
                await ItemsClone(equipType.id, index);
#endif
            }
...
#else
        private async Task ItemsClone(string typeId, int equipIndex) {
#endif
...
#else
            await PossessionEquips(typeId, cloneItem, equipIndex);
#endif
        }
...
#else
        private async Task PossessionEquips(string id, GameObject cloneItem, int equipIndex) {
#endif
            //各種初期化
...
#else
                    var isMatch = await ItemManager.CanEquipType(_runtimeActorDataModel, weaponDataModels[i].basic.weaponTypeId);
#endif
...
#else
                        equips.GetComponent<WindowButtonBase>().OnFocus.AddListener(async () =>
#endif
                        {
...
#else
                            await EquipSelect(id, parameters);
#endif
                        });
...
#else
                    await equipmentItems.EquipWindow(data);
#endif
...
#else
                    var isMatch = await ItemManager.CanEquipType(_runtimeActorDataModel, armorDataModels[i].basic.armorTypeId);
#endif
...
#else
                    await EquipSelect(equipTypeId, null);
#endif
                });
                equips.GetComponent<WindowButtonBase>().OnClick = new Button.ButtonClickedEvent();
                equips.GetComponent<WindowButtonBase>().OnClick.AddListener(() =>
...
#else
            await _equipMenu.EqValuesDisplay(parameters, equipTypeId, _selectIndex);
#endif
        }
...
#else
        public async Task EquipChange(string itemId) {
            //武器防具装備IDと装備タイプIDと武器か防具かが入ってくる
            await _equipMenu.EquipChange(itemId, _selectIndex);
        }
#endif
...
#else
        public async Task RemoveEquipment() {
            await _equipMenu.RemoveEquip(_selectIndex);
        }
#endif
