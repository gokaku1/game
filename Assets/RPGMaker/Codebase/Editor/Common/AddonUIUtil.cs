...
namespace RPGMaker.Codebase.Editor.Common.AddonUIUtil
{
    ...
    public class AddonUIUtil
    {
        ...
        public static string GetEventReplacedText(
            AddonInfo addonInfo,
            AddonParamInfo paramInfo,
            string typeName,
            string value,
            string srcMapId,
            string destMapId,
            Dictionary<string, string> eventMapping
        ) {
            var arrayDimension = 0;
            string structName = null;
            if (typeName == null) typeName = paramInfo.GetInfo("type")?.value ?? "string";
            ...
            if (arrayDimension == 2)
            {
                ...
                for (var i = 0; i < jsonArray.Count; i++)
                {
                    ...
                    var jsonArray2 = jsonArray[i].AsArray;
                    for (var j = 0; j < jsonArray2.Count; j++)
                        list2.Add(GetEventReplacedText(addonInfo, paramInfo, baseTypeName, jsonArray2[j].ToString(), srcMapId, destMapId, eventMapping));
                    list1.Add($"[{string.Join(',', list2)}]");
                }
            ...
            if (arrayDimension == 1)
            {
                ...
                var list1 = new List<string>();
                for (var i = 0; i < jsonArray.Count; i++)
                    list1.Add(GetEventReplacedText(addonInfo, paramInfo, baseTypeName, jsonArray[i].ToString(), srcMapId, destMapId, eventMapping));
                return $"[{string.Join(',', list1)}]";
            }
            ...
            if (structName != null)
            {
                ...
                {
                    var key = paramParamInfo.name;
                    if (!jsonObject.HasKey(key)) continue;
                    list1.Add(
                        $"\"{key}\":{GetEventReplacedText(addonInfo, paramParamInfo, paramParamInfo.GetInfo("type")?.value, jsonObject[key].ToString(), srcMapId, destMapId, eventMapping)}");
                }
                ...
                case ParamType.MapEvent:
                    {
                        ...
                        if (valueArray[0] == srcMapId)
                        {
                            valueArray[0] = destMapId;
                            value = DataConverter.GetJsonStringArray(valueArray);
                        }
                        if (eventMapping.ContainsKey(valueArray[1]))
                        {
                            valueArray[1] = eventMapping[valueArray[1]];
                            value = DataConverter.GetJsonStringArray(valueArray);
                        }
                        break;
                    }
