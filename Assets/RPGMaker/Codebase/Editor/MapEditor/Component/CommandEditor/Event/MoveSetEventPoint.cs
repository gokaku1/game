    ...
    public class MoveSetEventPoint : AbstractCommandEditor
    {
        ...
        public override void Invoke() {
            ...
            if (EventCommand.parameters.Count == 0)
            {
                ...
                EventCommand.parameters.Add("0"); //イベント選択2
        ...
        {
            //U267 このイベントの表示設定を行なう為
            var EventID = EventCommand.parameters[5];
            if (EventCommand.parameters.Count >= 9)
            {
                if (EventCommand.parameters[8] == "-1")
                {
                    //先頭に設定する
                    EventCommand.parameters[5] = "-1";
                }
            }
