...
namespace RPGMaker.Codebase.Runtime.Common
{
    public class SoundManager : MonoBehaviour
    {
        ...
        const string _bgmLoopInfoFilename = "Assets/RPGMaker/Storage/Sounds/bgmLoopInfo.json";
        const string _bgsLoopInfoFilename = "Assets/RPGMaker/Storage/Sounds/bgsLoopInfo.json";
#endif
...
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
        private static bool _processingInitLoopInfo = false;
        private static List<TaskCompletionSource<bool>> _waitingList = new List<TaskCompletionSource<bool>>();
#endif
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
        private void InitLoopInfo() {
#else
        public static async Task InitLoopInfo() {
#endif
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
            if (_processingInitLoopInfo)
            {
            ...
            if (_bgsFilenameLoopInfoDic == null)
            {
                ...
                LoadLoopInfo(1);
#else
                await LoadLoopInfo(1);
#endif
            }
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
            _processingInitLoopInfo = false;
...
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
                if (_systemSettingDataModel != null && !_systemsBgmSound.ContainsKey(SoundBgmNum.Title))
#endif
...
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
                if (DataManager.Self().GetRuntimeSaveDataModel() != null && !_systemsBgmSound.ContainsKey(SoundBgmNum.Battle))
#endif
...
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
                if (DataManager.Self().GetRuntimeSaveDataModel() != null && !_systemsBgmSound.ContainsKey(SoundBgmNum.Victory))
#endif
...
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
                if (DataManager.Self().GetRuntimeSaveDataModel() != null && !_systemsBgmSound.ContainsKey(SoundBgmNum.Defeat))
#endif
...
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
                if (_systemSettingDataModel != null && !_systemsBgmSound.ContainsKey(SoundBgmNum.GameOver))
#endif
...
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
                    if (_systemSettingDataModel != null && !_systemsSeSound.ContainsKey((SoundSeNum) i))
#endif
...
#else
                sounddata = await UnityEditorWrapper.AssetDatabaseWrapper.LoadAssetAtPath<AudioClip>(
#endif
                    path + clipName);
#if USE_PARTIAL_LOOP
...
#pragma warning restore CS1998 // 非同期メソッドは、'await' 演算子がないため、同期的に実行されます
        ...
        public void ChangeBgmState(int volume) {
            ...
            if (_bgmSoundCommon == null)
                //U369 オプションのボリュームを設定
                _bgmClip.volume = _runtimeConfigDataModel.bgmVolume / 100f * volume / 100f;
            else
                //U369 オプションのボリュームを設定
                _bgmClip.volume = _runtimeConfigDataModel.bgmVolume / 100f * _bgmSoundCommon.volume / 100f * (volume / 100f);
        }
        ...
        public void ChangeBgsState(int volume) {
            ...
            //U369 オプションのボリュームを設定
            _bgsClip.volume = _runtimeConfigDataModel.bgsVolume / 100f * _bgsSoundCommon.volume / 100f * (volume / 100f);
        }
        ...
        public void ChangeMeState(int volume) {
            ...
            //U369 オプションのボリュームを設定
            _meClip.volume = _runtimeConfigDataModel.meVolume / 100f * _meSoundCommon.volume / 100f * (volume / 100f);
        }
        ...
        //U326 MEを強制的にストップして、再生中のMEも破棄する
        public void ReleaseStopMe() 
        {
            _meClip.Stop();
            _isPlayingMe = false;
            _meClip.clip = null;
...
#endif
            //U359 参照対応
            if (_seSoundCommon == null || _seSources == null)return;

#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
            if (_runtimeDataManagementService == null)
            {
                Init();
            }
#endif
...
#else
                    sounddata = await AddressableManager.Load.LoadAssetSync<AudioClip>("Assets/RPGMaker/Storage/Sounds/" + folderName + "/" + filename);
#endif
#if USE_PARTIAL_LOOP
        ...
        public void ChangeSeState(int volume) {
            for (int i = 0; i < _seSources.Count; i++)
                ...
                {
                    //U369 オプションのボリュームを設定
                    _seSources[i].volume = _runtimeConfigDataModel.seVolume / 100f * _seSoundCommon.volume / 100f * (volume / 100f);
                }
        }
...
#else
        public static async Task LoadLoopInfo(int index) {
#endif
...
#else
            loopInfoModels = (await AddressableManager.Load.LoadAssetSync<LoopInfoSO>(filename)).dataModels;
#endif
#endif
...
#else
        static void SaveLoopInfo(int index) {
#endif
            var dic = (index == 0) ? _bgmFilenameLoopInfoDic : _bgsFilenameLoopInfoDic;
            var filename = (index == 0) ? _bgmLoopInfoFilename : _bgsLoopInfoFilename;
        ...
        public void UpdateLoopInfo(int index, List<string> deletedFilenames, List<string> importedFilenames) {
            InitLoopInfo();
#else
        public async Task UpdateLoopInfo(int index, List<string> deletedFilenames, List<string> importedFilenames) {
            await InitLoopInfo();
#endif
            var dic = (index == 0) ? _bgmFilenameLoopInfoDic : _bgsFilenameLoopInfoDic;
...
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
                _ = UpdateLoopInfo(typeDeletedFilenames, typeImportedFilenames);
            }
        }
        static async Task UpdateLoopInfo(List<string>[] typeDeletedFilenames, List<string>[] typeImportedFilenames) {
#endif
...
#else
                        await soundManager.UpdateLoopInfo(i, typeDeletedFilenames[i], typeImportedFilenames[i]);
#endif
                    }
                }
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
