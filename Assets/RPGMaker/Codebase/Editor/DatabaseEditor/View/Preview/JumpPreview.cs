    ...
    public class JumpPreview : AbstractPreview
    {
        ...
        protected override string GetAssetId() {
            ...
            else if (_targetId == "-1")
            {
                ...
                if (data != null)
                {
                    if (data.pages[0].condition.image.enabled == (int) EnabledType.SelectedImage)
                        assetId = data.pages[0].image.sdName;
                    else if (data.pages[0].condition.image.enabled == (int) EnabledType.Character)
                    {
                        var characters = database.LoadCharacterActor();
                        for (int i = 0; i < characters.Count; i++)
                    ...
                    else if (data.pages[0].condition.image.enabled == (int) EnabledType.Tile)
                        assetId = data.pages[0].image.tileName;
                }
            }
            // 指定イベント
            else
